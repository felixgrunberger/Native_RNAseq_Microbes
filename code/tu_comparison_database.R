...
# LOAD LIBRARIES
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
packages <- c("UpSetR","CoverageView", "tidyverse", "here", 
              "ggthemes", "data.table", "ggExtra", "Rsamtools", 
              "GenomicAlignments", "seqTools", "Rsubread", "ape", 
              "DT", "ggpubr", "ggridges", "ggsci")
...
sub_tus_ecoli <- fread(here("data/operon_data/ecoli_tex_suboperons.tsv"))
ecoli_collapsed <- fread(here("data/operon_data/ecoli_tex_suboperons.tsv")) %>%
...
#................................load Nanopore detected subTUs
sub_tus_pfu <- fread(here("data/operon_data/pfu_tex_suboperons.tsv"))
pfu_collapsed <- fread(here("data/operon_data/pfu_tex_suboperons.tsv")) %>%
  dplyr::rename(operon_number = operon_id, genes = genes_in_operon) %>%
  ungroup() %>%
...
#................................database
gff_table_gene <- read.gff(here("data/genome_data/hvo.gff")) %>%
  as_tibble() %>%
...
gff_table_cds <- read.gff(here("data/genome_data/hvo.gff")) %>%
  as_tibble() %>%
...
gff_table_combined <- left_join(gff_table_cds, gff_table_gene, by = "GeneID") %>%
...
operon_prediction_hvo <- fread(here("data/operon_data/hvo_database_operons.opr")) %>%
  dplyr::left_join(gff_table_combined, by = c("Synonym" = "old_locus_tag")) %>%
  dplyr::rename(operon_number = 1, genes = ID) %>%
  dplyr::select(operon_number, genes) %>%
  group_by(operon_number) %>%
  mutate(genes = toString(genes)) %>%
  distinct(operon_number, genes)
...
#................................load Nanopore detected subTUs
sub_tus_hvo <- fread(here("data/operon_data/hvo_tex_suboperons.tsv"))
hvo_collapsed <- fread(here("data/operon_data/hvo_tex_suboperons.tsv")) %>%
  ungroup() %>%
  dplyr::rename(operon_number = operon_id, genes = genes_in_operon) %>%
